(function($) {
  ...
  return $(function() {
		...
		if ( $('.modal iframe').attr('src').indexOf('autoplay=1') >= 0 ) {
			var url = $('.modal iframe').attr('src');
			$('.modal iframe').attr('data-af4-autoplay', '1').attr( 'src', url.replace( '&autoplay=1', '' ) );
		}
		...
		$('.modal').on('on.zf.toggler', function(e){
			...
			var $iframe = $(this).find('iframe');
			var url = $iframe.attr('src');
			$(this).fadeOut(1000, function(){ $iframe.attr( 'src', url.replace('&autoplay=1','') ); });
		});
		return;
  });
})(jQuery);
